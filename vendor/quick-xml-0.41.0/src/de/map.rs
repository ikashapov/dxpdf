...
/// (Clark notation, `{uri}local`) is one of `fields`, return it. This is how
/// a field opts in to namespace-qualified matching — by spelling its
/// `#[serde(rename = "...")]` that way — while every other field, whose
/// rename is a plain local name, is completely unaffected: the qualified
/// string this builds is never a member of `fields` for them, so
/// `next_key_seed` falls through to the original local-name-only
/// `QNameDeserializer::from_elem` exactly as upstream. Returns `None` (not
/// an error) whenever nothing qualifies — no namespace resolves, the
/// qualified name isn't declared, or the bytes aren't valid UTF-8 — so this
/// can only ever narrow behavior back to stock quick-xml, never introduce a
/// new failure mode.
///
/// A free function, not a method on `ElementMapAccess`, and called in its
/// own statement *before* `next_key_seed`'s main `match self.de.peek()? {`
/// rather than from inside one of that match's arms: the arms already hold
/// `e`, a reference borrowed from `self.de` for the arm's whole body, and
/// borrowing `self.de` a second time from inside the same match to resolve
/// the namespace would conflict with it. Pre-resolving into an owned
/// `Option<String>` sidesteps that — the borrow this function takes on
/// `reader` ends when the function returns, before the main match ever
/// starts.
fn resolve_qualified_key<'de, R: XmlRead<'de>>(
    reader: &R,
    fields: &'static [&'static str],
    start: &BytesStart<'de>,
) -> Option<String> {
    let ns = reader.resolve_element_namespace(start)?;
    let ns = std::str::from_utf8(&ns).ok()?;
    let local = start.local_name();
    let local = reader.decoder().decode(local.into_inner()).ok()?;
    let qualified = format!("{{{ns}}}{local}");
    if fields.contains(&qualified.as_str()) {
        Some(qualified)
    } else {
        None
    }
    ...
    E: EntityResolver,
{
    ...
    fn next_key_seed<K: DeserializeSeed<'de>>(
        ...
        if let Some(a) = self.iter.next(slice).transpose()? {
            ...
            // dxpdf patch (see `PATCH.md`): resolved into an owned value
            // *before* the main peek/match below, and not from inside one of
            // that match's arms — see `resolve_qualified_key`'s own doc for
            // why. `peek()` is idempotent (cached), so calling it here and
            // again just below sees the same event both times.
            //
            // The `.clone()` matters, not just for style: `e`, as returned by
            // `peek()`, is a reference whose *own* lifetime is tied to
            // `self.de`'s mutable borrow (elided from `&mut self`), distinct
            // from the `'de` its pointee's `Cow`s carry — so a fresh access
            // to `self.de.reader.reader` while `e` is still in scope conflicts
            // with that borrow. Cloning dereferences through it into an owned
            // `BytesStart<'de>` tied only to the input's `'de`, ending
            // `self.de`'s borrow right here, before that access is made.
            let start = match self.de.peek()? {
                DeEvent::Start(e) => Some(e.clone()),
                _ => None,
            };
            let qualified = start.as_ref().and_then(|start| {
                resolve_qualified_key(&self.de.reader.reader, self.fields, start)
            });
            ...
            match self.de.peek()? {
                ...
                // dxpdf patch (see `PATCH.md`): a field opted in to
                // namespace-qualified matching claims this element ahead of
                // both the `$value` catch-all and the ordinary local-name
                // match below — otherwise its qualified name, which is never
                // equal to the bare local name `not_in` checks against,
                // would make `not_in` see it as unnamed and misroute it.
                DeEvent::Start(_) if qualified.is_some() => {
                    self.source = ValueSource::Nested;

                    // unwrap: the guard just confirmed this is `Some`.
                    let de = StringDeserializer::<DeError>::new(qualified.unwrap());
                    seed.deserialize(de).map(Some)
                }
