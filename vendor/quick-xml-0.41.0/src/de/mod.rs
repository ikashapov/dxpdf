...
pub trait XmlRead<'i> {
    ...
    /// dxpdf patch (see `PATCH.md`): the namespace URI bound to `start`'s
    /// prefix, resolved the same way an element name resolves (`use_default
    /// = true`), or `None` when nothing is bound. Lets `ElementMapAccess`
    /// offer a namespace-qualified match for a field whose `#[serde(rename)]`
    /// opts in with Clark notation (`"{uri}local"`), without changing how
    /// any other field — which never uses that notation — is matched.
    fn resolve_element_namespace(&self, start: &BytesStart) -> Option<Vec<u8>>;
}
...
impl<'i, R: BufRead> XmlRead<'i> for IoReader<R> {
    ...
    fn resolve_element_namespace(&self, start: &BytesStart) -> Option<Vec<u8>> {
        match self.reader.resolver().resolve_element(start.name()).0 {
            crate::name::ResolveResult::Bound(ns) => Some(ns.into_inner().to_vec()),
            _ => None,
        }
    }
}
