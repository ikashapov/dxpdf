...
reads the paragraph's cascade (see `paragraph_locale` in
`src/render/layout/build/convert.rs`, which also records the one assumption
in it), while a list label reads its own §17.9.23 cascade, level `w:rPr`
first — the same layer its font and colour come from.
...
| §17.18.59's other ~50 formats — `hebrew1`, `japaneseCounting`, `chineseCounting`, `thaiLetters`, `arabicAlpha`, … | `st_enums.rs` (`StNumberFormat::Other`) | All degrade to `decimal` (`russianUpper`/`russianLower` graduated to real Cyrillic sequences in `numbering.rs`). Numbering-system and spellout data |
