...
§17.6.22: a page shared with a following `Continuous` section falls in the **succeeding** section's `page_range` — the last section on a page owns its header and footer. The rule, and the fact that ECMA-376 does not settle it, is stated at the decision site in `render::shared_page_owner_bounds`.
